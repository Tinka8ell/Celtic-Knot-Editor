...
# cell.py
"""
Cell - a representation of a square cell in matrix that is
a knot.  
"""
from typing import Dict, TypeVar

# as the editor could be run inside the "editor" module ... 
try:
...
class Cell:
    ...
    def __init__(self, x, y):
        self.x = x
        self.y = y
        self.walls = {Direction.North: None, Direction.East: None,
                      Direction.South: None, Direction.West: None}
        self.neighbours = {Direction.North: None, Direction.East: None,
                           Direction.South: None, Direction.West: None}
    ...
    def isEmpty(self) -> bool:
        ...
        for direction in Direction:
            isEmpty = isEmpty and (
                self.wall(direction) == Doorway.blocked or 
                self.wall(direction) == Doorway.removed)
        return isEmpty

    def changeWall(self, direction: Direction, wall: Doorway, 
                   symmetries: Symmetry) -> None:
        current = self.walls[direction]
        if not current:
            current = Doorway.blocked
        # ## print("Calling changeWall:", self.x, self.y, 
        #          direction.name, wall.name + ": current = ", current.name)
        if current != wall:
            # has changed, so change related walls
            # print("Changing:", self.x, self.y, 
            #       "changeWall(", direction.name + ",",  
            #       wall.name + ", symmetries):
            ...
            if neighbour:
                ...
                # ## print("set neighbour", neighbour.x, neighbour.y,  
                #          oppositeDirection.name, oppositeWall.name)
            ...
            if symmetries & Symmetry.Horizontal:
                ...
                if direction == Direction.East or direction == Direction.West:
                    ...
                    # horizontal reflection does not effect vertical facing walls
            ...
            if symmetries & Symmetry.Vertical:
                ...
                if direction == Direction.North or direction == Direction.South:
                    ...
                    # horizontal reflection does not effect horizontal facing walls
                    oppositeDirection = direction
                # ## print("paired:", paired.x, paired.y, Symmetry.Vertical.name)
                # do neighbours, but not symmetries (to avoid infinite recursion)
                paired.changeWall(oppositeDirection, wall, Symmetry.No)
            # ## print((symmetries & Symmetry.Rotate180), "or", 
            #          (symmetries & Symmetry.Rotate90))
            if ((symmetries & Symmetry.Rotate180) or 
                (symmetries & Symmetry.Rotate90) or 
                ((symmetries & Symmetry.Horizontal) and 
                 (symmetries & Symmetry.Vertical))):
                # 90 maps 180 as well as 90 and 270 below, 
                # and Horizontal and Vertical together also hits 180 ...
                paired = self.symmetry(Symmetry.Rotate180)
                oppositeDirection = direction.opposite
                # ## print("paired:", paired.x, paired.y, 
                #          Symmetry.Rotate180.name, "or", Symmetry.Rotate90.name)
                # do neighbours, but not symmetries (to avoid infinite recursion)
                paired.changeWall(oppositeDirection, wall, Symmetry.No)
