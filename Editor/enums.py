...
# enums.py
"""
The enums used to define various object properties.
"""
...
class Reverse:
    """
    Decorator class to add the opposite attribute 
    using the mapping of enum names
    """
    ...
    def __call__(self, enum):
        # use the match pairs in reverse_map 
        # to link enum names that are opposites of each other
...
class Rotate:
    """
    Decorator class to add the cw (clockwise) and ccw (counter clockwise) 
    attributes using the mapping of enum names
    """
    ...
    def __call__(self, enum):
        # each mapping is of clockwise links, 
        # so reverse mapping gives counter clockwise
...
# as decorators are for different attribute 
# we can apply one on top of the other and add both sets of attributes
...
class Doorway(IntEnum):
    ...
    # note that int value 6 has no Doorway defined, 
    # so not all int values in the range are valid
...
class Ligature(Enum):
    ...
    H = Doorway.head
    B = Doorway.beak
    # added lowercase versions, 
    # so will save making uppercase for some and introduce alternatives
...
if __name__ == "__main__":
    ...
    for d in Direction:
        ...
        n = Direction(i).name
        print(n, Direction[n], Direction[n].name,
              Direction[n].value, n == Direction[n].name)
    for d in Direction:
        print("Dir:", d.name, "Op:", d.opposite.name, 
              "Rot:", d.cw.name, "Rot:Op:", d.ccw.name,
