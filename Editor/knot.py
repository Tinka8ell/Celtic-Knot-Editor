...
# knot.py
"""
Knot - a representation of a matrix of Cells that is
a knot.  
"""
    ...
    def linkCells(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                cell.setSymmetry(Symmetry.Rotate180,
                                 self.cell(xmirror, ymirror))
    ...
    def cell(self, x: int, y: int) -> Cell:
        ...
        if self.width > x >= 0:
            if self.height > y >= 0:
                ...
                raise Exception(
                    "More rows than allowed: cell(" + str(x) 
                    + "," + str(y) + ")")
        else:
            raise Exception(
                "Line longer than allowed row: cell(" 
                + str(x) + "," + str(y) + ")")
