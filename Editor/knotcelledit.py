...
# kinotcelledit.py
"""
A frame representing a Cell to allow it to be modified
by a user.  Each of the 4 walls (or edges) can be set to 
any of the valid types for a Cell and the new Cell returned
to the editor to update the knot it came from.
"""
...
class KnotCellEdit(Frame):
    ...
    def create_widgets(self) -> None:
        ...
        self.northWall = self.addCombo(self.frame, list(
            self.doorways), chars, 0, 1, lambda event: self.changeNorthWall())
        self.eastWall = self.addCombo(self.frame, list(
            self.doorways), chars, 1, 2, lambda event: self.changeEastWall())
        self.southWall = self.addCombo(self.frame, list(
            self.doorways), chars, 2, 1, lambda event: self.changeSouthWall())
        self.westWall = self.addCombo(self.frame, list(
            self.doorways), chars, 1, 0, lambda event: self.changeWestWall())
        self.reset()
        return
    ...
    def changeWall(self, doorway, direction):
        ### print("cell editor calling change wall:", 
        ###       direction.name, doorway.name)
        if self.cell:
    ...
    def set(self, selected, callback, symmetries):
        ...
        self.symmetries = symmetries
        self.cell = selected
        self.northWall.current(self.doorways.index(
            selected.wall(Direction.North).name))
        self.eastWall.current(self.doorways.index(
            selected.wall(Direction.East).name))
        self.southWall.current(self.doorways.index(
            selected.wall(Direction.South).name))
        self.westWall.current(self.doorways.index(
            selected.wall(Direction.West).name))
        self.show()
        return
